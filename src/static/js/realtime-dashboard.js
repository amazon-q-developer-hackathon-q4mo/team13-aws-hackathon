...
class RealtimeDashboard {
    ...
    initializeCharts() {
        ...
        // 유입경로 바 차트
        this.charts.referrer = new Chart(document.getElementById('referrer-chart'), {
            type: 'bar',
            data: {
                labels: [],
                datasets: [{
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            afterBody: (context) => {
                                ...
                                const percentage = ((value / total) * 100).toFixed(1);
                                return [`비율: ${percentage}%`, `총 유입: ${total}명`];
                            }
                        }
                    }
                },
    ...
    updateReferrerChart(referrerData) {
        const labels = referrerData.labels || [];
        const data = referrerData.data || [];

        this.charts.referrer.data.labels = labels;
        this.charts.referrer.data.datasets[0].data = data;
        this.charts.referrer.update();
    }
