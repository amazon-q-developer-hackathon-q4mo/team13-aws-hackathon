...
5. **Phase 9 백업/재해복구 실패**:
   ```bash
   # 백업 상태 확인
   aws dynamodb describe-continuous-backups --table-name LiveInsight-Events
   # 재해복구 테스트
   ./scripts/test-phase9.sh
   # 운영 자동화 로그 확인
   aws logs filter-log-events --log-group-name /aws/lambda/liveinsight-auto-scaling
   ```
...
# Phase 9 배포 후 검증
./scripts/deploy-phase9.sh && ./scripts/test-phase9.sh
